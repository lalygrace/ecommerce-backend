...
export const createCoupon = async (data: CreateCouponDtoType) =>
  repo.createCoupon(data);

export const getCouponById = async (id: string) => repo.findCouponById(id);

export const getCouponByCode = async (code: string) =>
  repo.findCouponByCode(code);

export const listCoupons = async (opts: {
  page: number;
  limit: number;
  active?: boolean;
...
export const validateAndApply = async (payload: ApplyCouponDtoType) => {
  ...
  if (
    ...
    payload.totalCents < coupon.minOrderAmountCents
  )
  ...
  if (
    ...
    coupon.applicableCategorySlugs.length > 0
  ) {
    const categories = payload.categorySlugs ?? [];
    const has = categories.some((c) =>
      coupon.applicableCategorySlugs.includes(c),
    );
  ...
  if (coupon.type === 'PERCENTAGE') {
    discount = Math.floor(
      (payload.totalCents * (coupon.valueCents || 0)) / 100,
    );
  ...
  return {
    valid: true,
    coupon,
    discountCents: Math.min(discount, payload.totalCents),
  };
};
